...
In the *Asset List*, assets are added via drag & drop from the Finder. Accepted are audio files `.wav`, `.aif` and `.ogg`,
and Pure Data patches `.pd` (lowercase extensions; `.aiff` is not recognised, rename to `.aif`).
Dropped files are **copied** into the project's `assets` folder, the originals stay untouched,
so you can edit the project's copy without affecting your source material.
If a file of the same name already exists in `assets`, the existing file is used and the dropped one is *not* copied:
rename before dropping if you mean a different file.
...
or by clicking and dragging over the assets to be selected.
Their attributes then can be edited together.
An asset can be removed from a state with the ++delete++ key, if it is selected.
++shift++ + double-click opens the asset's file in the default application (Pd for patches, your audio editor for sound files);
a plain double-click lets you rename the asset.
...
| ------------------------------- | ------------- | ------ | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Playback Mode                   | Binaural-Mono | N/A    | *Binaural-Mono/-Stereo/-5Channel/-7Channel*: rendered with HRTFs depending on azimuth, elevation and distance to the listener (multichannel modes place the channels around the asset center, see *Channel Radius*). *Mono/Stereo*: plain playback, distance damping applies but no panning, for material whose spatial cues are already in the recording. |
...
| Fade-In Time                    | 50            | ms     | Ramp from silence to full gain when the asset starts.                                                                                                                                                                                                                                                                                                      |
| Fade-Out Time                   | 50            | ms     | Ramp to silence once the asset is stopped (e.g. on leaving the state). The fade starts on leave, so the asset remains audible for this long *after* the state has been left.                                                                                                                                                                               |
| Crossfade Time                  | 2000          | ms     | Overlap for looping: a second player starts this long before the end of the file, with an equal-power crossfade. Loops therefore don't need to be edited seamlessly. Must be shorter than the file.                                                                                                                                                        |
| Offset Time                     | 0             | ms     | Offset playback start.                                                                                                                                                                                                                                                                                                                                     |
| Gain (dB)                       | 0             | dB     | Gain factor for this asset (trim / boost)                                                                                                                                                                                                                                                                                                                  |
| Altitude                        | 0.0           | m      | Height of the asset above the listeners altitude.                                                                                                                                                                                                                                                                                                          |
...
| Fixed Orientation               | -1.0          | deg    | Assets keeps the same orientation relative to the player after entering the corresponding state, independent from the specified asset coordinates. Neither head yaw nor pitch is applied to it. Decimal values (e.g. 45.5) are allowed.                                                                                                                                                                                                         |
...
| Fixed Distance                  | -1.0          | m      | Asset stays at specified distance after entering the state, independent from the specified asset coordinates. Decimal values are allowed.                                                                                                                                                                                                                                              |
| Min Distance                    | -1.0          | m      | Minimal possible distance to the given asset. -1 disables the floor: the damping curve is followed continuously all the way down to 0 m.                                                                                                                                                                                                                                                                                                              |
...
| Raw Sensors to Pd               | false         | toggle | **No effect.**                                                                                                                                                                                                                                                                                                                                             |
| GPS to Pd                       | false         | toggle | **No effect**: the hero position is sent to every Pd patch asset anyway (`$0-lat`, `$0-lon`), see [Working with Pure Data](../faq/working-with-pd.md).                                                                                                                                                                                                     |
...
| Headtracker relative to source  | true          | toggle | In editing mode, the asset's playback responds relative to the movement of the connected headtracker.                                                                                                                                                                                                                                                      |
| Lock asset position             | false         | toggle | The asset can not be dragged around in the Asset Map View.                                                                                                                                                                                                                                                                                                 |
...
| Always play from start          | true          | toggle | Playback begins from the start of the file each time the state is entered. When disabled, the asset remembers where it was stopped and resumes from there on the next activation. Starting the simulation (or the walk on the phone) resets all positions.                                                                                                 |
